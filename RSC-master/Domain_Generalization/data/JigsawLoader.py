...
# def get_split_dataset_info(txt_list, val_percentage):  #JiGen加载txt文件
...
class JigsawNewDataset(data.Dataset):
    def __init__(self, names, labels, jig_classes=100, img_transformer=None, tile_transformer=None, patches=True,
        ...
        self.N = len(self.names)  #
        # self.permutations = self.__retrieve_permutations(jig_classes) #JiGen 里面的重组的
        #self.grid_size = 3    #JiGen里的 这里用不到
        self.bias_whole_image = bias_whole_image
        if patches:
            self.patch_size = 64
        self._image_transformer = img_transformer
    ...
    def __retrieve_permutations(self, classes):   #带有__函数表明是私有函数
        all_perm = np.load('permutations_%d.npy' % (classes))
        # from range [1,9] to [0,8]
        if all_perm.min() == 1:
            all_perm = all_perm - 1
